...
function novoElementoId(): string {
  contadorElemento += 1;
  return `elemento-continuo-${contadorElemento}`;
}
...
// 13.2b) ou um item avulso (não pertence a nenhum Conjunto). Gap identificado
// pelo Maestro no contrato: `/ambientes` não tinha nenhum mecanismo de
// seleção até esta task (só o handle de junção, que alterna união/quebra,
// não seleciona). Decisão: lista simples clicável (shadcn `Table`, mesmo
// padrão já usado no resto desta página) em vez de estender o hit-testing do
// `BoxCanvas` — menor risco, reaproveita o padrão de tabela já dominante
// aqui, e o handle de junção já é a única interação de canvas hoje.
...
export default function AmbientesPage() {
  ...
  // Elemento Contínuo), mesmo padrão de carga de `app/page.tsx`.
  const [catalogo, setCatalogo] = useState<Catalogo | null>(null);

  // Seleção de Conjunto/item avulso (ver `SelecaoAlvo` acima) e os Elementos
  // Contínuos adicionados no laboratório (local-first, mesmo espírito de
  // `itensColocados`/`overrides` — persistência real é Fase C futura).
  ...
  // Só relevante quando novoTipoElemento === "tamponamento" e a seleção é um
  // Conjunto com 2+ itens — qual módulo do bloco recebe o tamponamento (ver
  // decisão de "extremidade exposta" no JSX do painel, mais abaixo).
  ...
  // horizontal" nenhuma (Conjunto é puramente horizontal, Seção 3.3 do Modelo
  // de Domínio — o comentário de `AlvoResolvido` deixa essa lacuna explícita)
  // — decisão adotada aqui: qualquer módulo do bloco pode receber base/topo
  // independentemente (o painel/base de um módulo do meio está tão exposto
  // verticalmente quanto o da ponta; só a lateral é que fica encostada no
  // vizinho). Um item avulso (fora de Conjunto) expõe os 4 lados.
