...
>(({ className, children, ...props }, ref) => (
  // Task 13.2a — Design-System.md Seção 6.7 (mesmos tokens do Input),
  // substituindo os genéricos `border-input`/`text-sm`/`ring-ring` do stock.
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-9 w-full items-center justify-between whitespace-nowrap rounded-sm border border-cinza-300 bg-cinza-0 px-[10px] text-corpo text-cinza-900 shadow-none data-[placeholder]:text-cinza-400 focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent-subtle disabled:cursor-not-allowed disabled:border-cinza-200 disabled:bg-cinza-100 disabled:text-cinza-400 [&>span]:line-clamp-1",
      className
    )}
    {...props}
  >
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-2 pr-8 text-corpo outline-none focus:bg-accent-subtle focus:text-accent data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className
    )}
    {...props}
  >
