// Task 5.10-back/5.10-front (Modelo-de-Dominio.md 7.2) — mesmo padrão de mock
// local de `@/lib/supabase/server` já usado em `lib/orcamento/congelar.test.ts`.
// Movido de `etapa-esteira.test.ts` quando `atualizarEtapaEsteira` foi
// extraída para arquivo próprio (correção pós-build: Server Action precisa
// de "use server" no topo do arquivo, não inline, para ser importável por
// Client Component).
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { atualizarEtapaEsteira } from "./atualizarEtapaEsteira";
