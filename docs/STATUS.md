...
> Atualizado em 2026-08-17 (Task 5.1-5.4 mesclada, Lote 5 5/5 — **Épico V2.1/Fase D concluído**). Este arquivo é o ponto de partida de qualquer
...
- **Operador**: aplicar migration real `storage_organizacao_logos` (Task 4.8–4.9 back)
  via MCP ou Dashboard Supabase — migration `20260812130418_storage_organizacao_logos.sql` foi aprovada em audit mas não pode ser aplicada via CLI (mesmo bloqueio de permissão de conta).
- **Operador**: aplicar migration real `perfil_foto_url_storage` (Task 4.11 back)
  via MCP ou Dashboard Supabase — migration `20260813140434_perfil_foto_url_storage.sql` foi aprovada em audit mas não pode ser aplicada via CLI (mesmo bloqueio de permissão de conta).
- **Operador**: aplicar migration real `orcamento_cliente_id_no_action` (Task 4.15)
  via MCP ou Dashboard Supabase — migration `20260813160000_orcamento_cliente_id_no_action.sql` foi aprovada em audit mas não pode ser aplicada via CLI (mesmo bloqueio de permissão de conta).
- **Operador**: aplicar migration real `perfil_update_por_coluna` (Task 4.15 — **prioridade alta**, fecha vulnerabilidade de elevação de privilégio — revoke de escrita em coluna `papel`)
  via MCP ou Dashboard Supabase — migration `20260813170000_perfil_update_por_coluna.sql` foi aprovada em audit mas não pode ser aplicada via CLI (mesmo bloqueio de permissão de conta).
