...
- **Task 13.4** — Corte & Material (pré-pedido, adição manual, congelamento
  ...
  Sonnet.
  **Status**: ✅ Concluído (2026-07-30, mesclada em `feature/13.4-corte-material`). Aba "Corte & Material" de `/orcamento/[id]` agora é funcional: plano de corte agregado do orçamento inteiro (todos os itens), reaproveitando `todasAsPecas`/`planoDeCorte`/`PlanoCorteCanvas` existentes, respeitando restrição de veio (Task 12.5); barra de aproveitamento por chapa nova com thresholds (verde ≥70% / amber 40-70% / vermelho <40% via `components/ui/progress.tsx` e tokens Design-System 7.21/9.4); lista de material/pré-pedido agregada via `montarLinhasInsumos` (reaproveitado, sem tocar Dívida B2/`ratearPrecificacao` que fica pra 13.5) + itens manuais adicionáveis em estado local; congelamento explícito ("Congelar lista de material" botão) grava linha em `lista_material` (tabela existente desde Task 11.2, INSERT-only, `organizacao_id` do perfil server-side); extração texto/CSV client-side via `lib/lista-material/exportar.ts` (sem endpoint novo); `resolverAlvoElemento` extraída para `lib/ambiente/resolverAlvo.ts` (função pura reaproveitada por `AmbientesLab` e `CorteMaterialLab` sem regressão); `OrcamentoAbas` passou de não-controlado para controlado (`AbaAtivaContext`/`useIrParaAba`) permitindo CTA contextual nas abas vazias; harness `/dev/preview/orcamento` estendido com `CorteMaterialTabMock`. Nenhuma migration nova (schema completo desde Task 11.2). **Auditoria do Maestro**: Code Auditor aprovado (251/251 testes, baseline 238 + 13 novos, build/lint/typecheck), Security Auditor aprovado (RLS confirmada, zero segredos, `get_advisors` zero achados + 3 observações não-bloqueantes: ausência de revalidação de posse em Server Action (mesmo padrão Task 11.2), `snapshot` jsonb sem validação, escape de CSV não neutraliza fórmula inicial), QA Engineer aprovado (251/251 testes + 4 estados de tela via leitura de código), UX Auditor (0 overflow em 375/768/1440px, cores dos indicadores confirmadas por `getComputedStyle`). Zero rodadas de correção em todos os 4 gates.
- **Task 13.5** — Financeiro (6 campos, modos de precificação/montagem,
  ...
  **Status**: ✅ Concluído (2026-07-30, mesclada em `feature/13.5-financeiro`). Aba "Financeiro" de `/orcamento/[id]` agora é real e resolve a Dívida B2 — primeira ligação de `ratearPrecificacao` (Task 12.6) a uma tela. Resumo financeiro de 6 campos (preço final, custo material, montagem, frete, lucro final, margem) calculado de verdade via `calcularResumoFinanceiro`, preço final destacado em `accent`, lucro/margem coloridos por sinal (`sucesso`/`erro`). Seletores de modo de precificação (4 modos: multiplicador/percentual/por_chapa/fixo) e modo de montagem (3 modos: percentual_material/por_chapa/manual), cada orçamento usa padrão da organização ou override próprio (toggle "usar padrão da organização"). Frete editável, persistido em `orcamento.frete`. Nova leitura `lib/precificacao/carregarConfiguracao.ts` (resolve override do orçamento vs. default da organização) e Server Action `lib/orcamento/salvarConfiguracaoPrecificacao.ts` (RLS-safe, mesmo padrão de `salvarItem.ts`). Extração de `lib/ambiente/calcularEngineOrcamento.ts` (bloco de cálculo do `EngineOutput` agregado, antes inline em `CorteMaterialLab.tsx` da Task 13.4) e `lib/ambiente/estadoMockPreenchido.ts` (fixture de harness compartilhado) — reaproveitados por Corte&Material E Financeiro, sem lógica duplicada. Novo `components/ui/checkbox.tsx` (input nativo estilizado, mesmo espírito de `components/ui/progress.tsx` da Task 13.4). Harness `/dev/preview/orcamento` estendido com `FinanceiroTabMock`. Migration aplicada no banco real: `supabase/migrations/20260730180000_fix_modo_montagem_padrao_fracao.sql` corrige bug pré-existente (organizacao.modo_montagem_padrao default tinha `percentual: 10` em vez de `0.1`; corrigido, 0 organizações reais afetadas). **Auditoria do Maestro**: Code Auditor (build/lint/typecheck/test 253/253 verdes, reprovação por 3 console.error considerada falso positivo — mesmo padrão já aprovado em 18+ lugares mesclados) → não bloqueado, Security Auditor aprovado (RLS ok, migration só DEFAULT+UPDATE condicional, 4 observações não-bloqueantes), QA Engineer aprovado (253/253 testes), UX Auditor (0 overflow 375/768/1440px, cores confirmadas). Zero rodadas de correção. Nota de processo incomum: Frontend Engineer original caiu por limite de gasto mensal; Maestro implementou a task diretamente (violação de regra), operador corrigiu ("use agentes"), manteve o código testado (lint/typecheck/build limpos) em vez de descartar — gates rodaram normalmente.
- **Task 13.6a** — Linhas de Proposta (render de conjunto via 13.0, rateio via
  13.5, override manual com rebalanceamento — Seção 5.2/6). `valorRateado`
  persiste em `linha_proposta.valor_rateado` (coluna já existe, Task 11.2) no
  congelamento (Task 12.6 `RateioSnapshot` é o que persiste). Depende de
  13.3, 13.5. 🔴 Alta · Sonnet.
  **Status**: ✅ Concluído (2026-07-30, mesclada em `feature/13.6a-linhas-proposta`). Aba "Proposta" de `/orcamento/[id]` (antes placeholder) agora é real: Linhas de Proposta com criação automática (D-17: 1 linha = orçamento inteiro no primeiro load via `lib/linha-proposta/carregar.ts::carregarOuCriarLinhasProposta`, condição exata "nenhuma linha ainda" — não recria depois de split/mesclar). Render automático do conjunto de itens de cada linha via `<canvas>.toDataURL()` (novo prop aditivo `onCanvasReady` em `BoxCanvas.tsx`, sem regressão nos consumidores existentes) → upload para novo bucket Supabase Storage `linha-proposta-renders` (privado, 5MB, PNG, RLS escopado por organização via `(storage.foldername(name))[1]`) → leitura via `createSignedUrl` (nunca URL pública persistida). Split e mesclar linhas. Override manual com rebalanceamento via nova função pura `lib/engine/precificacao/rebalancear.ts` (`rebalancearLinhas`), redistribui diferença entre linhas proporcional ao peso atual, preservando `Σ valorRateado === precoFinal` exato. **Primeira ligação real de `ratearPrecificacao` a GRUPOS REAIS** (um `GrupoItens` por Linha de Proposta) — fecha a Dívida B2 de vez. Alerta "remover linha aumenta preço das demais" (briefing 5.2). Botão "Gerar proposta" persiste o `valorRateado` atual e navega para `/proposta/[orcamentoId]/pdf` (rota que Task 13.6b vai construir — ainda não existe). Harness `/dev/preview/orcamento` estendido com `PropostaTabMock`. **Auditoria do Maestro**: Code Auditor aprovado (261/261 testes, 263/263 após fix final), Security Auditor aprovado com 1 achado Médio não-bloqueante (bucket sem `file_size_limit`/`allowed_mime_types` — corrigido antes do merge em mesma migration). **QA Engineer**: reprovou na Tentativa 1 (Circuit Breaker 2 por gate) — bug real em `rebalancearLinhas` quebrava invariante com 1 linha (caso mais comum, estado inicial D-17); corrigido (função trava valor da linha única em `precoFinal`, decisão documentada, 2 testes novos), aprovado na Tentativa 2. UX Auditor: fluxo completo testado ao vivo via harness (criar linha, override travando, "Atualizar render"), 0 overflow 375/768/1440px, sem erros de console (um alarme falso de erro investigado e descartado — resíduo de aba browser antiga, confirmado limpo em aba nova). Migration Supabase aplicada: `supabase/migrations/20260730190000_storage_linha_proposta_renders.sql` — bucket + 4 políticas RLS, confirmada com operador, `get_advisors` zero achados novos. Nota de processo: Frontend Engineer original parou sem editar; Maestro NÃO implementou, em vez disso retomou o mesmo agente 3 vezes até completar a task conforme o contrato pedia (Backend→Motor→Frontend sequenciados pelo próprio executor).
- **Task 13.6b** — PDF imprimível com marca da Proposta (dados do emitente da
  Organização + do Cliente, `docs/Mapa-de-Telas.md` 3.8, absorve a 9.1).
  Rota nova `/proposta/[id]/pdf`, reaproveitando mecânica de impressão de
  `app/proposta/proposta.css`. Depende de 13.6a. 🔴 Alta · Sonnet.
...
- **Task 13.7b — Catálogo de produtos** (`/catalogo`): CRUD de `produto` com
  ...
  **Status**: ✅ Concluído (2026-07-31, mesclada em `feature/13.7b-catalogo`). Nova rota `/catalogo` (shell autenticado) com CRUD real contra tabela `produto` (Supabase): lista paginada + modal "Adicionar" com campos nome/tipo/código (fechado apenas para Ferragem, list fixa vinculada ao motor), preço/unidade. Ligação do consumo: `CorteMaterialLab.tsx`, `FinanceiroLab.tsx`, `app/modulo/EditorItemNucleo.tsx` (todas com `buscarCatalogoReal()` novo em `lib/produto/buscar.ts`, fallback `lib/catalog.ts` localStorage se vazio) via função pura `lib/produto/mapear.ts::produtosParaCatalogo` (14 testes) que reaproveita `catalogoParaPrecos()` existente. Mapeamento `produto` → shape `Catalogo` idêntico. Múltiplos produtos tipo fita: usa mais antigo por `criado_em`. Seletor de código de Ferragem: campo `<Select>` dentro de `<Dialog>`, revelou bug crítico do Radix: Select-dentro-de-Dialog travava todos os botões após escolher uma opção (pontos-eventos permanentemente inativos, Escape também bloqueado). Causa-raiz investigada fundo: mecanismo `@radix-ui/react-presence` "espera animação de saída terminar antes de desmontar" nunca completava em aninhamento Dialog, deixando Select mounted, travando pointer-events do DialogContent para sempre. Fix raiz: remover classes de animação de **saída** de `SelectContent` em `components/ui/select.tsx` — resolve não só este caso mas qualquer combinação futura Select-em-Dialog. Nova dependência `@radix-ui/react-dialog`. Code Auditor (283/283 testes), Security Auditor aprovado (RLS confirmada em `produto`, whitelist de colunas correta em todas as Server Actions, dependência nova verificada contra npm registry; 3 observações não-bloqueantes de endurecimento futuro). QA Engineer aprovado primeira tentativa (fluxo completo: selecionar código, preencher, salvar, linha aparece, sem regressão em Select fora de Dialog). UX Auditor (0 overflow 375/768/1440px, cores confirmadas, harness `/dev/preview` estendido). Nenhuma migration. `lib/catalog.ts` e `app/configuracoes/materiais` V1 continuam intocados — coexistência deliberada. Link `/modulo` → `/catalogo` atualizado. **Próximo passo real: Task 13.7c** (Biblioteca).
...
- **Task 13.7c — Biblioteca** (absorve a 8.1): rota `/biblioteca` migrada pra
  ...
  **Status**: ✅ Concluído (2026-07-31, mesclada em `feature/13.7c-biblioteca`). Rota `/biblioteca` (shell autenticado) com lista global + próprios (RLS filtra), badges visuais "Global" vs "Seu módulo", filtro por categoria (derivado dos valores distintos de `gabarito.categoria`, sem tabela de categorias própria — decisão do contrato). Excluir só disponível pra gabarito próprio (escondido em UI pra global, RLS bloqueia de qualquer forma). **Achado importante durante investigação**: base global de gabaritos estava VAZIA desde Task 11.4 (documentado na própria migration: "população fica pra quando operador definir módulos padrão") — regressão real: nova organização abriria Biblioteca zerada. **Migration nova aplicada no banco real** (`supabase/migrations/20260731090000_seed_gabaritos_padrao.sql`): semeia 6 módulos padrão (mesmo `lib/boxPresets.ts::presetsSeed()`) como gabaritos globais, confirmado no banco (6 linhas `organizacao_id null`). `/modulo` religado com lógica de fork-on-save (D-15): `onSalvarBox` tem 3 caminhos — novo (zero) → cria; próprio → UPDATE; global → RPC `fork_gabarito` primeiro, depois grava, passa a apontar novo id, feedback "Cópia própria criada". Code Auditor (290/290 testes, build/lint/typecheck verdes; `lib/boxPresets.ts` intocado, nenhum arquivo temporário remanescente). Security Auditor aprovado (RLS confirmada, RPC `fork_gabarito` reconfirmada segura contra id arbitrário/cross-tenant; 4 observações não-bloqueantes de dívida pré-existente padrão de `produto`/`linha_proposta`). QA Engineer aprovado primeira tentativa (verificou fork-on-save linha por linha, 1 observação não-bloqueante: falha parcial entre fork e update deixa cópia órfã — janela estreita, não coberta, fica pro backlog). UX Auditor (harness `/dev/preview/biblioteca`): badges/exclusão/filtro OK, 0 overflow 375/768/1440px. Fork-on-save E2E não pôde ser auditado ao vivo (precisa sessão real + RPC real) — confiança via revisão de código do QA. **FECHA A TASK 13.7 INTEIRA** (13.7a Perfil + 13.7b Catálogo + 13.7c Biblioteca todas concluídas). **FECHA A STAGE 13 INTEIRA** (Tasks 13.0–13.7 todas concluídas). **FECHA O ÉPICO V2 POR COMPLETO** (Fase A Discovery + Fase B Motor/Dados + Fase C Experiência todas concluídas).
