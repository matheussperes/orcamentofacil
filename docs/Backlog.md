...
- UI de cadastro do tamanho de rolo de fita (`Catalogo.tamanhoRoloFitaM`) e
  exibição dos rolos calculados (`calcularRolosDeFita`) na lista de material.
  Motor e tipos completos (Task 3.6, 2026-08-11); UI deliberadamente deixada
  de fora do escopo desta task por não ter tela de front companheira agendada —
  operador pode cadastrar via Supabase Table Editor até task futura de UI.
  Candidato a task futura de captura + exibição visual, sem impacto em cálculo
  ou lógica.
- Peças no plano de corte (`PlanoCorteCanvas.tsx`) continuam com cor azul
  estática em vez de espelhar `material.claro/medio/escuro` conforme
  Design-System §9.4. Lacuna pré-existente separada (não é novo achado nesta
  rodada, mas foi registrado incidentalmente durante a Task 3.2, 2026-08-11).
  Candidato a task futura de harmonização visual — impacto baixo, UI apenas,
  sem mudança de motor ou persistência.
...
- `fitaPorCor` (lib/engine/consolidar.ts, usado para lista de compra de fita
  discriminada por cor) nunca recebe a parcela de fita de Elemento Contínuo
  (Tampo) — `peca.fita_m` de peças de Tampo é sempre 0 por design (fita é
  calculada agregada no nível do módulo em `explodeTampo`), mas `somaFita` só
  soma `peca.fita_m` por peça. `fitaTotalM` soma `mod.fitaM` corretamente, mas
  `fitaPorCor` fica sem nenhuma entrada de fita de Tampo para qualquer
  orçamento com Elemento Contínuo. Gap arquitetural pré-existente, não
  introduzido por Tasks 3.10–3.11/3.12. Achado incidental durante a Task 3.12
  (2026-08-12). Candidato a task futura de motor: propagar cor+espessura-final
  do módulo até `fitaMap` quando `peca.fita_m===0` mas `mod.fitaM>0`.
- `EditorItemNucleo.tsx` e componentes-filho (`CaixaCard.tsx`, `DivisoesCard.tsx`,
  `PortasCard.tsx`, etc., em `app/modulo/`) continuam com CSS legado (`.card`,
  `.legado-grid`, `app/globals.css`) mesmo após migração de `/modulo` para o
  shell/Design System v3 (Task 5.1–5.4, 2026-08-17). Núcleo é compartilhado
  com `/orcamento/[id]/item/[itemId]` (Editor de Item real, já em produção,
  já dentro do shell), e re-estilizar o núcleo inteiro é retrofit maior e mais
  arriscado — deixado de propósito fora do escopo da Task 5.1–5.4. Candidato
  a task futura de UI: harmonização visual do núcleo, sem impacto em motor ou
  lógica de negócio.

### Gaps de rótulo/UI registrados, sem task própria ainda

- **Tooltip primitivo não existe no projeto** (Task 4.15, observação não bloqueante,
  2026-08-13). Para o cenário de usuários não-admin (hoje inalcançável — toda
  organização tem 1 admin obrigatório por design), o Design-System recomenda
  botão "Excluir organização" **disabled** + Tooltip explicativo em vez de
  desaparecer totalmente. Como nenhum primitivo Tooltip está implementado no
  projeto e o cenário é impossível hoje, a correção ficou marcada como trabalho
  futuro. Candidato a task de UI quando/se o design de papéis mudar.
...
- Card "Elementos de parede" (`AmbientesLab.tsx`) usa "Aplicar preset" / "Nenhum preset salvo" / "Salvar como preset" como rótulo visível — mas essa é a entidade `ElementoParedePreset` (Modelo-de-Dominio D-29, Task 2.12), conceito distinto de "Preset = item posicionado" já resolvido para "Módulo" em task anterior (Task 2.13). Não tocado por Task 5.5-5.6 (fora do escopo). Candidato a task futura de consistência de vocabulário se o operador decidir que "preset" também deveria sumir dessa feature.
...
| 2.3–2.6 | Cadastrar/editar/ordenar ambientes e paredes dentro do orçamento; seletor de parede que expande o painel daquela parede; indicação visual permanente de qual ambiente e qual parede estão em edição | ✅ Completo | Lote 0 (0.1–0.3), Lote 1 | frontend-engineer (web) | Sonnet | Modelo 3.2, 11.5; PRD RF-19 |
...
| 2.7 | Motor: tipo de elemento de parede "pedra" + regra de bloqueio de conjunto por tipo (porta/janela quebram o bloco físico; pedra/tomada/ponto_hidráulico não quebram) + aviso `TAMPO_SOBRE_PEDRA` | ✅ Completo | Lote 0, Lote 1 | motor-engineer | Sonnet | Modelo 3.2.2 (tabela de bloqueio, exemplo trabalhado) |
| 2.8–2.11 (back) | Backfill de `id`/`refX`/`refY`/`nome` nos elementos de parede já em produção (neutro: `refX="esquerda"`, `refY="chao"` reproduz a leitura atual) + funções de conversão canônico ↔ referência de medida | ✅ Completo | Lote 0, Lote 1 | backend-engineer | Sonnet | Modelo 3.2.2; `schema-v2.1-delta.sql` §3 |
| 2.7–2.11 (front) | UI de elemento de parede: seletor do tipo "pedra", edição após adicionar (via linha da lista **e** via clique na representação 2D), referência de medida escolhível (X esquerda/direita, Y chão/teto) com **rótulos descritivos** ("Distância da parede esquerda", "Altura do chão" — nunca "X"/"Y") | ✅ Completo | 2.7, 2.8–2.11 (back), Lote 1 | frontend-engineer (web) | Sonnet | Modelo 3.2.2 (tabela de rótulos, exemplos trabalhados); PRD RF-25 |
| 2.12 (back) | `ElementoParedePreset`: nova tabela (só `nome` obrigatório + largura/altura de prefill), **fora do catálogo de produtos**, sem preço/status/tipo — decisão do operador que rejeitou a recomendação do backlog-fonte (Q-5) | ✅ Completo | Lote 0, Lote 1 | backend-engineer | Sonnet | Modelo 3.2.3; `schema-v2.1-delta.sql` §5; PRD D-29 |
| 2.12 (front) | UI de criar/listar/aplicar preset de elemento de parede (aplicar copia nome+dimensão, sem vínculo vivo) | ✅ Completo | 2.12 (back), Lote 1 | frontend-engineer (web) | Sonnet | Modelo 3.2.3; PRD RF-25 |
| 2.13 | Rename "Preset" → "Módulo" em toda a interface (identificador de domínio continua `preset`/o que for — é rótulo, não tipo) | ✅ Completo | Lote 0, Lote 1 | frontend-engineer (web) | Haiku | Design-System 15.3; PRD 10.4 item 1 |
...
| 2.19–2.23 | Editar módulo no momento da inserção, na mesma tela: largura/altura/profundidade, cor e espessura da caixa, cor e espessura das portas, fundo sim/não, tipo de puxador | ✅ Completo | Lote 0, Lote 1 | frontend-engineer (web) | Sonnet | PRD RF-05, RF-26 |
| 2.27 | Cotas à direita na elevação: altura total da parede + altura de cada faixa | ✅ Completo | 2.24–2.26, Lote 1 | frontend-engineer (web) | Sonnet | Modelo 11.5; PRD RF-27 |
| 2.28–2.30 | Agrupamento comercial cross-faixa/parede (`LinhaProposta`) com afordância visual distinta do bloco físico (`Conjunto`) na mesma elevação; botão de tag de conjunto disponível também nos módulos superiores; garantir que os dois **nunca colapsem no mesmo botão** (2.30 é critério de aceite desta task, não task separada) | ✅ Completo | 2.24–2.26, Lote 1 | frontend-engineer (web) | Sonnet | Modelo 3.3, 6; PRD RF-37, Q-3 |
...
- **Task 2.27** ✅ (2026-08-14) — Cotas técnicas à direita da elevação da parede: altura total (`parede.altura`, não `alturas.peDireito` — comentário no código justifica divergência possível) + altura de cada faixa (inferior/meio/aéreo). Função pura exportada `cotasFaixas(bandas)` com interface `CotaFaixa` em `components/ambientes/ElevacaoParede.tsx`, testada isoladamente com 3 valores exatos da fixture (900/500/1300mm, soma batendo com altura total 2700mm) e caso de borda de lista vazia. `MARGIN_RIGHT` do SVG alargado de 56 para 100, bracket da faixa "torre" deslocado para coluna mais externa (evita colisão das duas colunas de cota novas). Estilo: linha de extensão cinza-400, seta de medida, texto `tabular-nums text-cinza-700` — padrão técnico já usado em Editor de Item (Design-System §9.5). Sem prop nova em `ElevacaoParedeProps`, sem dado novo de schema/motor. `code-auditor` APROVADO (1ª tentativa), `qa-engineer` APROVADO (1ª tentativa, 626/626 testes, sem regressão), `ux-auditor` APROVADO (1ª tentativa, 3 colunas de cota sem sobreposição verificadas via `/dev/preview/orcamento`, conformidade §9.5 confirmada). Sem `security-auditor` (design puro). Impacto Visual: Leve.
- **Task 2.28–2.30** ✅ (2026-08-14) — Agrupamento comercial cross-faixa/parede com afordância visual distinta: investigação prévia (executor + code-auditor + qa-engineer) confirmou que 2.28 (cross-faixa/parede — `LinhaProposta.itens` já lista livre), 2.29 (handles de Conjunto funcionam todas as faixas — `porFaixa` sem filtro) e 2.30 (visual distinto — telas diferentes) já atendidos por código pré-existente. Gap real: badge comercial somente-leitura na aba Ambientes (elevação SVG + painel "Itens posicionados") mostrando vínculo de item a `LinhaProposta`, cor `accent` (#B45309), distinto de handle de Conjunto (`informacao` #2563EB). Regra de ruído: badge só aparece com 2+ linhas de proposta (padrão D-17, sem redundância). Função `derivarTagsComerciais` em `components/ambientes/AmbientesLab.tsx` (nova, testada 3 casos), prop `tagsComerciais` em `BoxCanvas`/`ElevacaoParede` (opcional, sem regressão 3 consumidores). Plumbing: `app/(app)/orcamento/[id]/page.tsx` → `AmbientesTabConectada` → `AmbientesLab`, reaproveitando `carregarOuCriarLinhasProposta` (sem schema novo). Harness `/dev/preview/orcamento` atualizado com mock 2+ linhas. `code-auditor` APROVADO (veredito em arquivo), `qa-engineer` APROVADO (629/629 testes, sem regressão), `ux-auditor` APROVADO (4 screenshots: badge visível 2 linhas, distinto de handle, sem regressão 3 consumidores, conforme Design-System). Nenhum Circuit Breaker. Uma retomada via SendMessage no meio da execução (nota de processo, não contada como tentativa). Impacto Visual: Completo. Sem migration. **Fecha o Lote 2 por completo — 18/18 tasks concluídas.**

**Lote 2 — 18/18 tasks concluídas** ✅
...
- **Task 3.2** ✅ (2026-08-11) — Investigação confirmou que `temVeio` já chegava correto até o plano de corte (`Peca.temVeio` → `expandirPecas()` → `PecaRetangular`/`PecaPosicionada`), sem falha de dados. Gap era 100% de rendering, sem raiz em motor. Novo componente `components/ui/switch.tsx` (button `role="switch"` nativo, precedente de `checkbox.tsx`, zero nova dependência Radix). Toggle "Mostrar veios" (`temVeio: true`) no cabeçalho da seção "Plano de corte" (`CorteMaterialLab.tsx`), desligado por padrão (UX deliberada — marceneiro vê informação coletada, mas legível só sob demanda). `PlanoCorteCanvas.tsx` desenha seta dupla fina horizontal (#475569/cinza-600, 1.5px, sem escala) centralizada em cada peça com `temVeio: true` quando toggle ativo — peças sem veio nunca mostram seta; seta é read-only (sem inversão, sem clique, nem tooltip — ausência de Server Action/persistência de `sentidoVeio` documentada no contrato como divergência deliberada da Design-System §9.4, campo não sobrevive até este ponto da pipeline). 532 testes (4 novos), zero regressão. `code-auditor` APROVADO (1ª tentativa), `qa-engineer` APROVADO (1ª tentativa, 4 casos `dedeDesenharSetaDeVeio` testados). `ux-auditor` APROVADO (1ª tentativa, 4 screenshots reais confirmando toggle off/on, seta presente/ausente por peça, ausência de cursor pointer — mock do harness não tinha peça com `temVeio: true` por padrão; auditor editou temporariamente o mock só para capturar evidência e reverteu antes de terminar, árvore de trabalho confirmada limpa). Sem `security-auditor` (sem RLS/segredo). Impacto Visual: Leve. Achado incidental: peças no plano de corte continuam com cor azul estática em vez de `material.claro/medio/escuro` (Design-System §9.4) — lacuna pré-existente separada (registrado em "Gaps de lógica/design", acima).
- **Task 3.5 (motor)** ✅ (2026-08-11) — Fita de borda discriminada por cor: nova agregação em `lib/engine/consolidar.ts` com função `larguraFitaMm()` implementando regra "menor fita ≥ espessura final" (busca em tabela `[22,35,65]`, sem hardcoding de faixas), função `somaFita()` agregando por Map chaveado `cor|largura`, novo tipo `GrupoFita { cor, larguraFitaMm, metros }`, novo campo `EngineOutput["consolidado"].fitaPorCor`. Campo `fitaTotalM` preservado inalterado — soma(fitaPorCor)===fitaTotalM verificada por cálculo aritmético manual. 478 testes (10 novos), zero regressão. `code-auditor` APROVADO (1ª tentativa), `qa-engineer` APROVADO (1ª tentativa, invariante soma verificada manualmente). Sem `security-auditor` (motor puro) nem `ux-auditor` (sem UI). **Desbloqueia Task 3.5 (front) e Task 3.6.**
- **Task 3.5 (front)** ✅ (2026-08-11) — Exibir fita discriminada por cor na lista de material: função `montarLinhasInsumos()` (`lib/insumos.ts`) reescrita para iterar sobre `engine.consolidado.fitaPorCor` em vez de uma única linha genérica "Fita de borda", gerando uma `LinhaInsumo` por cor+largura (`item: "Fita de borda ${cor} ${larguraFitaMm}mm"`); preço unitário permanece genérico (`precos.fitaMetro` — catálogo não distingue fita por cor); total recalculado como `metros * unitario` por linha. Sem linha zerada quando `fitaPorCor` está vazio (correção de UX anterior). Override de quantidade (Task 3.8) funciona sem alterações — a chave de merge mudou de `"Fita de borda"` para `"Fita de borda ${cor} ${larguraFitaMm}mm"`, mas `item_chave` permanece identificador único, sem regressão. 541 testes (5 novos), zero regressão. `code-auditor` APROVADO (1ª tentativa), `qa-engineer` APROVADO (1ª tentativa, soma de totais por cor+largura verificada manualmente, funcionamento de override confirmado), `ux-auditor` APROVADO (1ª tentativa real, screenshot com 2 linhas distintas: "Fita de borda Branco TX 22mm" e "Fita de borda Louro Freijó 22mm", padrão visual idêntico ao das linhas de MDF/Ferragens, checklist §15.4 12/12). Sem `security-auditor` (sem RLS/segredo). Impacto Visual: Leve. **Fecha a Task 3.5 (motor + front) por completo** — RF-29 (fita discriminada por cor) resolvido ponta a ponta. **Task 3.6 (rolos de fita a comprar) segue desbloqueada e pendente.**
- **Task 3.6** ✅ (2026-08-11) — Cálculo de rolos de fita a comprar: tipos `EspecificacaoFita.tamanhoRoloM?: number` em `lib/produto/tipos.ts` e `Catalogo.tamanhoRoloFitaM?: number` em `lib/catalog.ts`; derivação em `produtosParaCatalogo()` (`lib/produto/mapear.ts`) seguindo exatamente o mesmo critério já usado para `fitaMetro` (primeiro produto tipo fita ativo por `criado_em`, sem fallback inventado — ausente/inválido fica `undefined`); `calcularRolosDeFita(fitaPorCor, tamanhoRoloM)` em `lib/insumos.ts` implementa `Math.ceil(metros / tamanhoRoloM)` por grupo de cor/largura, array vazio quando `tamanhoRoloM` ausente/≤0. Validação `typeof === "number" && > 0` sem fallback. 551 testes (5 novos), zero regressão. `code-auditor` APROVADO (1ª tentativa), `qa-engineer` APROVADO (1ª tentativa, aritmética de arredondamento verificada manualmente). Sem `security-auditor` (sem RLS) nem `ux-auditor` (sem UI — formulário de cadastro do tamanho de rolo e exibição dos rolos calculados ficam para task futura, fora deste escopo; operador pode cadastrar via Supabase Table Editor até lá). Sem impacto visual. **Desbloqueia UI de rolos (task futura fora deste escopo).**
- **Task 3.10–3.11 (motor)** ✅ (2026-08-11) — Tampo: tipo `ModeloTampo` (`"simples" | "engrossado" | "dobrado"`) + `ConfigTampo` em `lib/engine/elemento-continuo/types.ts`; função `modeloDoTampo()` deriva `modelo` de `engrossamento` via projeção pura (não é campo persistido separado — decisão de design que torna invariante `modelo ⟺ engrossamento` impossível de violar); `ESPESSURAS_VALIDAS_POR_MODELO_TAMPO` com tabela completa (simples 15/18/25; engrossado/dobrado base 15 → 30/45/60; base 18 → 36/54); funções de validação e operação (`validarEspessuraTampo`, `espessuraFinalDoConfigTampo`, `validarConfigTampo`, `trocarModeloTampo` — limpa espessura ao trocar para modelo incompatível, nunca coage) em `lib/engine/elemento-continuo/explode.ts`; `explodeTampo` passou a calcular `fitaM` real (antes sempre 0 — correção legítima dentro do escopo, sem regressão em engrossado/dobrado). Todos os 3 exemplos numéricos do contrato (simples 25mm, engrossado nível1 base15 4 lados sarrafo70, dobrado nível1 base15 — com fita 3,02m) e 3 exemplos de rejeição (`ESPESSURA_INVALIDA_PARA_MODELO` ×2, `ESPESSURA_6MM_EM_TAMPO`) reproduzidos em teste com verificação aritmética manual. 490 testes (15 novos), zero regressão. `code-auditor` APROVADO (1ª tentativa), `qa-engineer` APROVADO (1ª tentativa, verificação aritmética manual do caso engrossado confirmada). Sem `security-auditor` (motor puro) nem `ux-auditor` (sem UI). **Desbloqueia Task 3.10–3.11 (front) e Task 3.12.**
- **Task 3.10–3.11 (front)** ✅ (2026-08-11) — UI do tampo em `AmbientesLab.tsx`: seletor de modelo (Simples/Engrossado/Dobrado) inserido **antes** do seletor de Espessura, só quando tipo de elemento contínuo é "tampo". Lista de espessuras filtrada por modelo via `ESPESSURAS_VALIDAS_POR_MODELO_TAMPO` (interseção com catálogo para simples, tabela fixa para engrossado/dobrado). Troca de modelo via `trocarModeloTampo` limpa espessura incompatível no estado (nunca coage). Construção de `engrossamento` extraída como função pura `montarEngrossamentoTampo(selecao)` exportada de `lib/engine/elemento-continuo/explode.ts`, com defaults documentados: 4 lados, sarrafo 70mm para engrossado — **2 rodadas de gate**: `qa-engineer` reprovou na 1ª tentativa por falta de teste automatizado da construção de engrossamento; corrigido com 4 casos em `lib/engine/elemento-continuo/explode.test.ts` (simples/engrossado 30mm/engrossado 54mm/dobrado 60mm), **APROVADO na 2ª tentativa**. `code-auditor` APROVADO (1ª tentativa), `ux-auditor` APROVADO (1ª tentativa, 6 screenshots reais confirmando modelo antes de espessura, lista mudando por modelo, campo limpo ao trocar modelo incompatível, ausência de seletor em rodapé/tamponamento/fechamento). Sem `security-auditor` (sem RLS/segredo). 563 testes, zero regressão. Impacto Visual: Leve. **Fecha a Task 3.10–3.11 (motor + front) por completo — RF-28 (modelo de tampo com UI e motor integrados) resolvido ponta a ponta.** Achado incidental: seletor visual de "quais lados engrossar" (Modelo 3.2.1, nota final) não implementado, fora de escopo — registrado em seção de gaps.
- **Task 3.8 (back)** ✅ (2026-08-11) — Persistência de override de quantidade por item da lista de material: migration `supabase/migrations/20260811110000_lista_material_override.sql` cria tabela `lista_material_override` com colunas `organizacao_id`, `orcamento_id`, `item_chave`, `quantidade`; RLS com SELECT/INSERT/UPDATE/DELETE escopadas por `organizacao_id = private.org_do_usuario()`, UPDATE com WITH CHECK simétrico; constraint UNIQUE `(orcamento_id, item_chave)` e CHECK `quantidade >= 0`. Server Actions `definirOverrideQuantidade`, `removerOverrideQuantidade` e função de leitura `listarOverridesQuantidade` em `lib/lista-material/override.ts`, ambas com checklist de posse (organização resolvida do servidor via `perfil` do usuário autenticado, orçamento cross-tenant rejeitado antes de qualquer escrita). Rejeição por orçamento congelado nas duas Server Actions de escrita, sem caminho de contorno. 506 testes (10 novos), zero regressão. `code-auditor` APROVADO (1ª tentativa), `security-auditor` APROVADO (1ª tentativa, RLS revisada linha a linha, checklist de posse confirmado, rejeição de congelamento sem bypass), `qa-engineer` APROVADO (1ª tentativa, 5 critérios + posse cruzada). Sem `ux-auditor` (sem UI). **Migration NÃO aplicada no Supabase real** — mesmo bloqueio de permissão de conta CLI já documentado; pendente do operador via MCP/Dashboard. **Desbloqueia Task 3.8 (front).**
- **Task 3.13 (catálogo-back)** ✅ (2026-08-11) — Bucket de Storage `texturas` (read-only para `authenticated`, escrita restrita a `service_role` — bucket curado, único e compartilhado por todas as organizações). Campo `texturaUrl?: string` em `EspecificacaoChapa` (`lib/produto/tipos.ts`). Validação anti-hotlink em `lib/produto/acoes.ts` (`ehCaminhoRelativoValido`): rejeita URL com esquema ou protocolo-relativo, aceita só caminho relativo dentro do bucket. **2 rodadas de gate**: `security-auditor` reprovou na 1ª tentativa — achado Alto: validação usava `.trim()` (remove espaço nas pontas), mas parser WHATWG do navegador remove C0 controls em qualquer posição, permitindo bypass `"htt\tps://evil.com"`. Corrigido com `.replace(/[\u0000-\u0020\u007f]/g, "")` removendo controles em qualquer posição; 2 testes novos cobrem 10 variantes de bypass rejeitadas + 6 casos válidos sem regressão. `code-auditor` APROVADO (1ª tentativa), `security-auditor` APROVADO (2ª tentativa, 10 payloads de bypass verificados), `qa-engineer` APROVADO (1ª tentativa, 504 testes, 8 novos). Sem `ux-auditor` (sem UI). **Migration NÃO aplicada no Supabase real** — mesmo bloqueio de permissão de conta CLI; pendente do operador via MCP/Dashboard. **Desbloqueia Task 3.13 (catálogo-front).**
- **Task 3.8 (front)** ✅ (2026-08-11) — UI de edição de quantidade na lista de material: `LinhaInsumo.quantidadeBase: number` exposto em `lib/insumos.ts`; função pura `aplicarOverridesQuantidade` em `lib/lista-material/aplicarOverrides.ts` (merge de overrides sobre linhas do BOM, recalcula `total`, não muta entrada, não altera categoria/item/unidade); edição inline de quantidade em `CorteMaterialLab.tsx` (só linhas do BOM, não itens manuais) com indicador visual Badge "Editado" quando override ativo, botão "Reverter" que remove override, edição desabilitada/oculta quando `congeladoEm !== null`, erro tratado com Alert inline sem quebra de tela. Threading de overrides + `congeladoEm` via `CorteMaterialTabConectada.tsx` e `app/(app)/orcamento/[id]/page.tsx`. `CorteMaterialTabMock.tsx` também ajustado como segundo consumidor real. Snapshot de congelamento (`montarSnapshotAtual`) usa valores com override aplicado. 524 testes, zero regressão. `code-auditor` APROVADO (1ª tentativa), `qa-engineer` APROVADO (1ª tentativa, funções puras testadas + edição/reversão via reducer testável), `ux-auditor` APROVADO (1ª tentativa, 8 screenshots reais confirmando indicador visual, recálculo de total, reversão, foco visível; 2 critérios não verificáveis no harness — edição desabilitada com congelamento e mensagem de erro simulada — confirmados por leitura de código, não bloqueantes). Impacto Visual: Leve. **Fecha a Task 3.8 (back + front) por completo — RF-15 (quantidade editável, valor/categoria/descrição travados) resolvido ponta a ponta.**
- **Task 3.4** ✅ (2026-08-11) — Contagem de cortes/passadas de serra: campo `Chapa.numCortes: number` em `lib/engine/box/cutting.ts`, computado por função exportada `calcularNumCortes(numPecas)` = `Math.max(0, numPecas - 1)`. Prova matemática: um corte de guilhotina sempre divide 1 região em exatamente 2 sub-regiões; por indução, separar N peças até 1 peça por folha custa sempre exatamente N-1 cortes, independente da geometria ou ordem dos cortes — propriedade de árvore binária (N folhas ⇒ N-1 nós internos), comentário explicativo no código. 4 testes novos: 1 peça → 0 cortes; 3 peças → 2 cortes (exemplo trabalhado); 0 peças → 0 cortes (borda); verificação cruzada contra reconstrução independente via `guilhotinavel()` num caso de 4 peças (confirma fórmula contra verificador real do domínio, não reafirma mesma fórmula). Fixture de `Chapa` mock em `lib/lista-material/tipos.test.ts` atualizada com campo novo obrigatório. 536 testes (4 novos), zero regressão. `code-auditor` APROVADO (1ª tentativa), `qa-engineer` APROVADO (1ª tentativa, verificação aritmética manual confirmada). Sem `security-auditor` (motor puro) nem `ux-auditor` (sem UI). Impacto Visual: Nenhum.
- **Task 3.13 (catálogo-front)** ✅ (2026-08-11) — Seletor de textura no formulário de cadastro/edição de produto tipo chapa: `lib/produto/texturas.ts` (nova) — `listarTexturasDisponiveis()` chama bucket Storage `texturas` read-only com `.list()` + `.getPublicUrl()`, sem upload/delete; Dialog aninhado `SeletorTexturaChapa.tsx` (novo) com grid de miniaturas, estados loading/vazio/preenchido, preview da textura selecionada, botão remover. Integração em `FormularioChapaDialog.tsx`: grava caminho relativo em `especificacao.texturaUrl`. Sem componente de galeria reutilizável no projeto — implementado como Dialog aninhado próprio (candidato a formalização futura se necessário). 528 testes, zero regressão. `code-auditor` APROVADO (1ª tentativa), `qa-engineer` APROVADO (1ª tentativa, casos de borda de bucket vazio/erro/filtro de pasta testados), `ux-auditor` APROVADO (1ª tentativa, checklist Design-System §15.4 completo 12/12, 6 screenshots reais incluindo estado vazio real do bucket do projeto e estados de seleção/preview/remoção simulados via interceptação de rede). Impacto Visual: Leve. 2 observações não bloqueantes: (1) Dialog aninhado é decisão pragmática não formalizada como padrão no Design-System — candidato a formalização futura; (2) alt text da miniatura usa caminho do arquivo, aceitável. **Fecha a Task 3.13 (catálogo-back + catálogo-front) por completo — consumo de textura real no catálogo resolvido ponta a ponta.** `ModuleViewer` (Task 3.13 front, RF-38, consumidor 3D) segue pendente, sem dependência bloqueante desta.
- **Task 3.12** ✅ (2026-08-12) — Investigação confirmou que o BOM dos 3 modelos de Tampo (simples/engrossado/dobrado, Modelo-de-Dominio.md Seção 3.4.1) e a regra de fita 25mm→35mm já estavam corretos como efeito colateral da Task 3.10–3.11 (motor) — sem divergência real, sem mudança de comportamento. Reforço de teste em `lib/engine/elemento-continuo/explode.test.ts`: os 3 exemplos trabalhados (simples 25mm; engrossado nível 1 base 15, 4 lados, sarrafo 70mm; dobrado nível 1 base 15 — mesmo bloco 1800×610) com verificação aritmética manual explícita e assert de largura de fita (não só metros); `larguraFitaMm(25)===35` confirmado já coberto em `consolidar.test.ts`. 3 exemplos de rejeição confirmados cobertos (pré-existentes). 563 testes (mesma contagem — assertions reforçadas em `it()` existentes, nenhum `it()` novo), zero regressão. `code-auditor` APROVADO (1ª), `qa-engineer` APROVADO (1ª). Sem `security-auditor` (motor puro) nem `ux-auditor` (sem UI). **Fecha a Task 3.12 por completo — Lote 3 em 13/14.** **Achado incidental para gap futuro registrado em "Gaps de lógica/design":** `fitaPorCor` (lib/engine/consolidar.ts, usado para lista de compra de fita discriminada por cor) nunca recebe a parcela de fita de Elemento Contínuo (Tampo) — `peca.fita_m` de peças de Tampo é sempre 0 por design (fita é calculada agregada no nível do módulo em `explodeTampo`), mas `somaFita` só soma `peca.fita_m` por peça. `fitaTotalM` soma `mod.fitaM` corretamente, mas `fitaPorCor` fica sem nenhuma entrada de fita de Tampo para qualquer orçamento com Elemento Contínuo. Gap arquitetural pré-existente, não introduzido por 3.10–3.11/3.12. Candidato a task futura de motor: propagar cor+espessura-final do módulo até `fitaMap` quando `peca.fita_m===0` mas `mod.fitaM>0`.
- **Task 3.13 (front)** ✅ (2026-08-12) — ModuleViewer: visualização 3D estática do módulo em edição, componente novo em `components/modulo/ModuleViewer.tsx`. Stack nova no projeto (`three`, `@react-three/fiber`, `@react-three/drei`, `@types/three`, instalada nesta task). Câmera ortográfica fixa (`isometric` default/`front`/`top`/`side`), sem `OrbitControls`, sem rotação livre — troca de ângulo instantânea via `useLayoutEffect` sem interpolação. Material com caminho único: `useTexture+SRGBColorSpace` quando há `textureUrl`, senão `meshStandardMaterial` color (nunca terceiro caminho). Geometria (`width`/`height`/`depth`) e cor (`corParaHex(box.caixa.cor)`) vêm da MESMA fonte que `BoxCanvas` já consome — nenhum segundo caminho de derivação. Integrado como segundo modo ("3D estático") do painel de visualização em `app/modulo/EditorItemNucleo.tsx` via `Tabs` underline, ao lado de "2D técnico" (default) — mesmo frame externo nos dois modos. Controles de ângulo próprios (4 botões `size="icon"`, distintos de Frontal/Traseira/Esquerda/Direita/Explodida do 2D). `next/dynamic({ ssr: false })` com skeleton de canvas técnico (`bg-cinza-50`/`border-cinza-200`, ícone `Box`) enquanto o bundle carrega. Campo `texturaUrl` adicionado a `AcabamentoMdf` (`lib/catalog.ts`) e populado em `produtosParaCatalogo` (`lib/produto/mapear.ts`) a partir de `especificacao.texturaUrl` do Produto tipo chapa (já alimentado pelas Tasks 3.13 catálogo-back/catálogo-front). Consumido nas duas telas que compartilham `EditorItemNucleo`: `/modulo` e `/orcamento/[id]/item/[itemId]` (confirmado por leitura de código — núcleo genuinamente compartilhado, sem duplicação). Responsivo, sem overflow abaixo de 768px. 563 testes, zero regressão (nenhum teste novo — componente sem lógica pura a extrair, mesmo padrão de `BoxCanvas` que também não tem teste de componente, só de matemática). `code-auditor` APROVADO (1ª tentativa), `qa-engineer` APROVADO (1ª tentativa, 10/10 critérios do contrato com prova por leitura de código), `ux-auditor` APROVADO (1ª tentativa, 13 screenshots reais: os dois modos do seletor, os 4 ângulos, loading skeleton sob throttle de rede, cor variando por material via `corParaHex`, responsivo mobile). Sem `security-auditor` (sem RLS/segredo novo). Impacto Visual: Completo. **Fecha a Task 3.13 (front) por completo — RF-38 (ModuleViewer 3D estático) resolvido ponta a ponta. Fecha o Lote 3 por completo — 14/14 tasks concluídas.**
...
| 4.6–4.7 | Máscara de CNPJ e de telefone (10 ou 11 dígitos) no formulário de perfil | ✅ Completo | nenhuma | frontend-engineer (web) | Haiku | PRD RF-31 |
| 4.8–4.9 (back) | Upload de logo: bucket de Storage + política de RLS + coluna de referência em `organizacao` | ✅ Completo | nenhuma | backend-engineer | Sonnet | PRD RF-02, RF-31, risco 12 |
| 4.8–4.9 (front) | UI de upload de logo substituindo o campo de URL; logo persiste e aparece em todos os lugares que a exibem (perfil, PDF da proposta) | ✅ Completo | 4.8–4.9 (back) | frontend-engineer (web) | Sonnet | PRD RF-17, RF-31 |
| 4.10 | Fallback: sem logo cadastrada, exibe a marca padrão Orça Fácil | ✅ Completo | 4.8–4.9 (front) | frontend-engineer (web) | Haiku | PRD RF-31 |
| 4.11 (back) | Upload de foto de perfil pessoal: bucket de Storage + coluna de referência | ✅ Completo | nenhuma | backend-engineer | Sonnet | PRD RF-31, risco 12 |
| 4.11 (front) | UI de upload de foto de perfil | ✅ Completo | 4.11 (back) | frontend-engineer (web) | Sonnet | PRD RF-31 |
| 4.12–4.13 (back) | Troca de senha com confirmação por e-mail (fluxo do Supabase Auth) | ✅ Completo | nenhuma | backend-engineer | Sonnet | PRD RF-31 |
| 4.12–4.13 (front) | UI da área de segurança dedicada no perfil, com o fluxo de troca de senha; e-mail permanece não editável (identificador da conta, item 4.14 — sem task própria, é constraint desta) | ✅ Completo | nenhuma | frontend-engineer (web) | Sonnet | PRD RF-31 |
| 4.15 | **Excluir conta = excluir a organização inteira** (Q-13, Modelo 7.3), disponível no perfil. Cascade completo de 12 tabelas (`organizacao` incluída) + duas correções que o cascade exige + a checagem de autorização por papel (Q-17, Modelo 7.3 "Quem pode disparar"). **Nesta ordem**: **(0)** a Server Action verifica `perfil.papel === 'admin'` do usuário autenticado **antes** de qualquer operação destrutiva — `vendedor`/`projetista`/sem sessão são rejeitados com `NAO_AUTORIZADO_EXCLUIR_ORG`/403, nada é apagado, nenhum passo abaixo roda; **(1)** migration — `orcamento.cliente_id` sai de `on delete restrict` para `on delete no action` (RESTRICT é verificado imediatamente e abortaria a cascata mesmo dentro da mesma transação); **(2)** Server Action/RPC `SECURITY DEFINER` lê os `perfil.id` da organização **antes** de apagar (depois eles não existem mais); **(3)** `delete from organizacao where id = ...` — a cascata (`on delete cascade` já presente em toda tabela de tenant) apaga sozinha `perfil` · `cliente` · `produto` · `gabarito` (só os da org — os globais, `organizacao_id is null`, sobrevivem) · `orcamento` e, por ele, `ambiente` · `parede` · `linha_proposta` · `lista_material` · `elemento_continuo` · `elemento_parede_preset`; **(4)** expurga os objetos de Storage sob o prefixo da organização (logo, fotos de perfil — sem FK, não morrem pela cascata); **(5)** chama a Admin API do Supabase Auth (`service_role`) para apagar cada usuário lido no passo 2. **Nenhuma política de `delete` nova em `organizacao` para `authenticated`** — a única porta é a Server Action, com a checagem de papel do passo 0 dentro dela | ✅ Completo | nenhuma | backend-engineer | Opus — não é CRUD padrão: cascade multi-tabela irreversível + Admin API (`service_role`) + expurgo de Storage sem FK, três superfícies fora do padrão RLS do resto do schema, com ordem de execução que precisa estar certa da primeira vez (sem undo) | Modelo 7.3 (cascade completo, "Quem pode disparar", 4 armadilhas técnicas, exemplo trabalhado, casos de borda); PRD RF-31, Q-13 (resolvida), Q-17 (resolvida) |

**Critérios de aceitação da Task 4.15** (não removível ao executar):
- [x] Usuário com `perfil.papel` `vendedor` ou `projetista` (ou sem sessão)
      ...
      acontece
- [x] Migration aplica `orcamento.cliente_id ... on delete no action` (ou
...
- [x] Excluir uma organização com dado real (perfis, clientes, orçamentos com
...
- [x] Objetos de Storage sob o prefixo da organização são removidos
- [x] Confirmação via `Dialog` explícito (não `window.confirm`) antes de
      disparar a exclusão — texto exato é nota para o `product-designer`
- [x] `security-auditor` revisou a task antes do merge — pré-requisito de
...
| 4.16 (front) | Campo "Espessura de serra (kerf)" no formulário de `/perfil`, editável, com aviso de retroatividade: mudar o valor altera o plano de corte de todo orçamento **não congelado** (mesma disciplina das alturas de faixa) | ✅ Completo | 4.16 (back) | frontend-engineer (web) | Sonnet | Modelo 8.2; PRD RF-02, RF-34, risco 12 |
...
**Histórico de execução:**
- **Task 4.4** ✅ (2026-08-12) — Migration `produto.codigo` (text, nullable) com constraint `unique (organizacao_id, codigo)` via `20260812120000_produto_codigo.sql`. Tipos em `lib/produto/tipos.ts` (novo campo `codigo?: string` em `Produto`), leitura em `lib/produto/acoes.ts`, gravação em `criarProduto`/`atualizarProduto` com tratamento de erro de violação de unicidade (Postgres `23505`) traduzido em mensagem amigável (`mensagemErroProduto`). Três testes novos em `lib/produto/acoes.test.ts`. `code-auditor` APROVADO (1ª tentativa, veredito em `.maestro/tmp/verdicts/4.4-code-auditor.md`), `qa-engineer` APROVADO (1ª tentativa, 566/566 testes, unicidade escopada e null-não-colide cobertos por prova de app + leitura da migration, veredito em `.maestro/tmp/verdicts/4.4-qa-engineer.md`). Sem `security-auditor` (sem RLS nova, RLS de `produto` já existente cobre a tabela). Sem `ux-auditor` (backend puro, sem UI — catálogo UI é a Task 4.1–4.3/4.5). **Migration real NÃO aplicada no Supabase** — mesmo bloqueio de permissão de conta CLI já documentado; pendente do operador via MCP/Dashboard. **Desbloqueia a Task 4.1–4.3/4.5 (Catálogo unificado).**
- **Task 4.1–4.3, 4.5** ✅ (2026-08-12) — Catálogo unificado (RF-30): `/catalogo` deixa de usar Tabs por categoria (Todos/Chapas/Ferragens/LEDs/Acessórios/Fita) e passa a ter um único card com seletor de categoria interno (Select), botão genérico "Adicionar item", e um único `FormularioProdutoDialog` parametrizado por categoria (campos específicos aparecem só depois de escolhida a categoria). Campo universal `codigo` (Task 4.4) exposto pela primeira vez na UI em todas as categorias. Status (Badge Ativo/Inativo + `AlternarAtivoDialog`) preservado. Categoria travada (read-only) na edição de produto existente. 6 arquivos antigos removidos (`FormularioChapaDialog.tsx`, `FormularioFerragemDialog.tsx`, `FormularioSimplesDialog.tsx`, `TabelaChapas.tsx`, `TabelaFerragens.tsx`, `TabelaSimples.tsx`), substituídos por `FormularioProdutoDialog.tsx` + `TabelaProdutos.tsx` (líquido -387 linhas). `code-auditor` APROVADO (1ª tentativa, veredito em `.maestro/tmp/verdicts/4.1-4.3-4.5-code-auditor.md`), `qa-engineer` APROVADO (1ª tentativa, 575/575 testes, 9 testes novos cobrindo lógica pura por categoria, veredito em `.maestro/tmp/verdicts/4.1-4.3-4.5-qa-engineer.md`), `ux-auditor` APROVADO (1ª tentativa, 11 screenshots reais via `/dev/preview/catalogo`, 3 breakpoints, Design-System §7.7/§7.9/§7.11 conformes, veredito em `.maestro/tmp/verdicts/4.1-4.3-4.5-ux-auditor.md`). 2 observações não bloqueantes: (1) modo escuro não implementado em lugar nenhum da aplicação — gap pré-existente; (2) estado vazio da tabela não foi capturável pelo harness (nenhuma categoria mock chega a zero itens), componente já existia antes. Design-System.md §7.8 (linha 528) ainda lista "abas do catálogo" — desatualização intencional, decisão de produto já registrada. Impacto Visual: Completo. Sem migration (mudança de UI pura).
...
- **Task 4.16 (front)** ✅ (2026-08-14) — Campo "Espessura de serra (kerf)" em `/perfil` (`SecaoOrganizacao` em `components/perfil/PerfilLab.tsx`): `<Input type="number" min={0}>` editável, ganho de `espessuraSerraPadraoMm` já lido e persistido via `organizacao` (Task 4.16 back, mesclada). Função pura `sanitizarKerf` (novo em `components/perfil/PerfilLab.tsx:209`, exportada) valida 0 como válido, rejeita negativo/NaN → 0, aplicada via `onChange`. Legenda de aviso de retroatividade: "mudar o valor altera o plano de corte de todo orçamento não congelado" — mesmo padrão de `alturasOverride` (Task 2.3–2.6 alturas). 4 testes novos em `components/perfil/PerfilLab.test.ts:83-101` (`describe("sanitizarKerf", ...)`) cobrindo casos (negativo, NaN, 0 válido, valores normais). `code-auditor` APROVADO (1ª tentativa, veredito em `.maestro/tmp/verdicts/4.16-front-code-auditor.md`), `qa-engineer` APROVADO (1ª tentativa, 633/633 testes, sem regressão, veredito em `.maestro/tmp/verdicts/4.16-front-qa-engineer.md`), `ux-auditor` APROVADO (1ª tentativa, 2 screenshots confirmando campo editável + legenda de aviso, Design-System conforme, veredito em `.maestro/tmp/verdicts/4.16-front-ux-auditor.md`, 2 screenshots adicionais em `.maestro/tmp/screenshots/`). Impacto Visual: Leve. Sem migration (schema já existia desde Task 4.16 back). **Fecha RF-02/RF-34 (kerf) ponta a ponta** — junto com a Task 4.16 (back). **Fecha o Lote 4 por completo — 13/13 tasks concluídas.**
- **Task 4.8–4.9 (back)** ✅ (2026-08-12) — Upload de logo: bucket de Storage privado `organizacao-logos` (2 MB, PNG/JPEG/WEBP, sem SVG), migration `supabase/migrations/20260812130418_storage_organizacao_logos.sql` com 4 políticas de RLS escopadas por organização, módulo `lib/organizacao/logo-storage.ts` (novo) com path determinístico `{organizacao_id}/logo.<ext>`, upload client-side com `upsert: true`, leitura via signed URL sob demanda (nunca persistida). Coluna `organizacao.logo_url` (pré-existente) reaproveitada (semântica muda de URL externa para PATH de objeto Storage), comentários atualizados em `lib/organizacao/salvar.ts`, `lib/perfil/carregar.ts`, `lib/proposta-pdf/carregar.ts`. `code-auditor` APROVADO (1ª tentativa, veredito em `.maestro/tmp/verdicts/4.8-4.9-back-code-auditor.md`), `security-auditor` APROVADO (1ª tentativa real — tivequeda de conexão na 1ª chamada não contada como tentativa, retomada via SendMessage com veredito completo em `.maestro/tmp/verdicts/4.8-4.9-back-security-auditor.md`, isolamento cross-tenant confirmado, sem segredo exposto, sem policy ampla), `qa-engineer` APROVADO (1ª tentativa, 580/580 testes, 5 novos cobrindo path determinístico e rejeição de SVG, veredito em `.maestro/tmp/verdicts/4.8-4.9-back-qa-engineer.md`). Sem `ux-auditor` (backend puro, sem UI — integração UI é Task 4.8–4.9 front, separada). **Migration real NÃO aplicada no Supabase** — bloqueio de permissão de conta CLI; pendente do operador via MCP/Dashboard. **Desbloqueia a Task 4.8–4.9 (front) e é pré-requisito da Task 4.10 (fallback de marca padrão).**
- **Task 4.8–4.9 (front)** ✅ (2026-08-12) — UI de upload de logo em `/perfil` (RF-31): campo de texto "Logo (URL da imagem)" removido de `SecaoOrganizacao` em `components/perfil/PerfilLab.tsx`, substituído por controle de upload de arquivo real (`input type="file" accept="image/png,image/jpeg,image/webp"` oculto, disparado por `Button`), consumindo diretamente `lib/organizacao/logo-storage.ts` (Task 4.8–4.9 back, mesclada). Upload dispara imediatamente na seleção (com estado de loading "Enviando…"), validação client-side de mime/tamanho antes do upload via função pura `validarArquivoLogo` (testada), preview resolvida via signed URL com fallback gracioso para valor legado (URL externa antiga) e placeholder `ImageOff` existente preservado para estado vazio. Persistência do path só acontece no clique de "Salvar alterações" (mesmo fluxo de dois passos dos demais campos da seção). `SecaoPerfilPessoal` (foto de perfil pessoal, Task 4.11) não foi tocada. `code-auditor` APROVADO (1ª tentativa, veredito em `.maestro/tmp/verdicts/4.8-4.9-front-code-auditor.md`), sem `security-auditor` (upload já protegido por RLS desde Task 4.8–4.9 back, nenhuma política nova aqui), `qa-engineer` APROVADO (1ª tentativa, 584/584 testes, 4 novos cobrindo validação de mime/tamanho, veredito em `.maestro/tmp/verdicts/4.8-4.9-front-qa-engineer.md`), `ux-auditor` APROVADO (1ª tentativa, 6 screenshots via `/dev/preview/perfil`, estados vazio/loading/erro capturados; estado "preenchido" com preview real não foi capturável no harness por exigir sessão Supabase autenticada — registrado como observação não bloqueante, veredito em `.maestro/tmp/verdicts/4.8-4.9-front-ux-auditor.md`). Impacto Visual: Leve. Sem migration. **Desbloqueia a Task 4.10 (fallback de marca padrão).**
- **Task 4.10** ✅ (2026-08-12) — Fallback de marca padrão no PDF de proposta: `lib/proposta-pdf/carregar.ts` ganhou `assinarUrlLogo` (réplica de `assinarUrlImagem`, função existente no mesmo arquivo para signed URLs de imagens de linha de proposta), resolvendo `organizacao.logo_url` (path de Storage desde Task 4.8–4.9 back) em signed URL no servidor, antes do primeiro paint. `logo_url` nulo ou path que falhe ao resolver (arquivo removido, valor legado de URL externa pré-Task-4.8–4.9) resulta em `null`, deixando o fallback visual **já existente** em `PropostaPdfCabecalho.tsx` assumir (`?? "/logo/logo-dark.png"`) — nenhuma mudança de componente visual, só a camada de dados server-side. Único lugar do produto que exibe a logo própria do marceneiro fora de `/perfil`; sidebar/login/signup usam a marca fixa do produto, fora de escopo. `code-auditor` APROVADO (1ª tentativa, veredito em `.maestro/tmp/verdicts/4.10-code-auditor.md`). Sem `security-auditor` (sem RLS/política nova). `qa-engineer` APROVADO (1ª tentativa, 587/587 testes, 3 novos cobrindo path válido/nulo/inválido, prova por contagem de chamadas de que o guard de nulidade evita I/O desnecessário, veredito em `.maestro/tmp/verdicts/4.10-qa-engineer.md`). `ux-auditor` APROVADO (1ª tentativa, screenshot via `/dev/preview/proposta-pdf` confirmando marca padrão renderizada com `logoUrl: null`; caso "com logo real" não capturável sem sessão autenticada, registrado como observação não bloqueante — mesma ressalva já registrada na Task 4.8–4.9 (front), veredito em `.maestro/tmp/verdicts/4.10-ux-auditor.md`). Impacto Visual: Leve. Sem migration. **Fecha a Task 4.10 por completo e com ela a Task 4.8–4.9 (back/front) — ciclo completo de RF-31 "upload de logo com fallback de marca padrão" resolvido ponta a ponta.**

- **Task 4.6–4.7** ✅ (2026-08-12) — Máscara de CNPJ e telefone: funções puras `formatarCnpj` e `formatarTelefone` novas em `lib/format.ts` (mesmo arquivo de `formatarMoeda`/`formatarDataHora`). Máscara progressiva (funciona com digitação parcial), lida com string suja (colar valor já formatado ou só dígitos), campo vazio permanece vazio, telefone troca automaticamente entre formato fixo (10 dígitos: `(XX) XXXX-XXXX`) e celular (11 dígitos: `(XX) XXXXX-XXXX`). Aplicadas via `onChange` nos 3 campos `org-cnpj`, `org-telefone`, `perfil-telefone` de `components/perfil/PerfilLab.tsx`. Nenhuma mudança de Server Action/schema — o valor formatado é a mesma string livre que já era salva antes. `code-auditor` APROVADO (1ª tentativa, veredito em `.maestro/tmp/verdicts/4.6-4.7-code-auditor.md`), sem `security-auditor` (formatação de texto, sem RLS/segredo), `qa-engineer` APROVADO (1ª tentativa, 13 testes escopados — suíte completa não exigida por não ser fim de stage, veredito em `.maestro/tmp/verdicts/4.6-4.7-qa-engineer.md`), `ux-auditor` APROVADO (1ª tentativa, screenshot via `/dev/preview/perfil` confirmando os 3 campos mascarados: CNPJ (`XX.XXX.XXX/XXXX-XX`), telefone celular 11 dígitos (`(XX) XXXXX-XXXX`), telefone fixo 10 dígitos (`(XX) XXXX-XXXX`), veredito em `.maestro/tmp/verdicts/4.6-4.7-ux-auditor.md`). Impacto Visual: Leve. Sem migration.
- **Task 4.11 (back)** ✅ (2026-08-13) — Upload de foto de perfil pessoal: bucket de Storage privado `perfil-fotos` (2 MB, PNG/JPEG/WEBP, sem SVG), migration `supabase/migrations/20260813140434_perfil_foto_url_storage.sql` com coluna `perfil.foto_url text nullable` e 4 políticas de RLS escopadas por usuário individual (`auth.uid()`, não por organização — colegas da mesma organização não podem ler/escrever a foto uns dos outros), módulo `lib/perfil/foto-storage.ts` (novo, réplica de `lib/organizacao/logo-storage.ts`) com path determinístico `{perfil_id}/foto.<ext>`, upload client-side com `upsert: true`, leitura via signed URL sob demanda. Coluna `PerfilCarregado.fotoUrl`/`DadosPerfil.fotoUrl` adicionadas em `lib/perfil/carregar.ts`/`lib/perfil/salvar.ts` (mesmo `.update()` já existente, sem Server Action nova). Ajustes mínimos em `PerfilMock.tsx`/`app/(app)/perfil/page.tsx` só para satisfazer o tipo (`fotoUrl: ""`), nenhuma UI nova. `code-auditor` APROVADO (1ª tentativa, veredito em `.maestro/tmp/verdicts/4.11-back-code-auditor.md`), `security-auditor` APROVADO (1ª tentativa, isolamento escopado por `auth.uid()` confirmado — não depende de `perfilId` enviado pelo client, só da RLS avaliada contra sessão real, veredito em `.maestro/tmp/verdicts/4.11-back-security-auditor.md`), `qa-engineer` APROVADO (1ª tentativa, 601/601 testes, 5 novos cobrindo path determinístico/upsert/isolamento/rejeição de SVG, veredito em `.maestro/tmp/verdicts/4.11-back-qa-engineer.md`). Sem `ux-auditor` (backend puro, sem UI — integração UI é Task 4.11 (front), separada). **Migration real NÃO aplicada no Supabase** — bloqueio de permissão de conta CLI; pendente do operador via MCP/Dashboard. **Desbloqueia a Task 4.11 (front).**
...
- **Task 4.12–4.13 (back)** ✅ (2026-08-13) — Troca de senha com confirmação por e-mail (RF-31, fluxo Supabase Auth): rota `app/auth/confirm/route.ts` (Task 13.3a pré-existente) estendida para `type=recovery` além de `type=email`, redireciona para `/perfil?definirSenha=1` em vez de `/` quando é recuperação de senha, mantendo o caminho `type=email` sem regressão. Duas Server Actions novas em `lib/auth/trocar-senha.ts`: `solicitarTrocaSenha()` (sem parâmetro, e-mail vem de `getUser()`, chama `resetPasswordForEmail`), `definirNovaSenha(novaSenha)` (valida com `senhaValida` reaproveitada de `lib/auth/validacao.ts`, chama `updateUser({password})`). Nova `mensagemErroTrocaSenha` em `lib/auth/mensagens.ts`, padrão de `mensagemErroLogin`/`mensagemErroSignup`. Nenhuma UI tocada. `code-auditor` APROVADO (1ª tentativa, veredito em `.maestro/tmp/verdicts/4.12-4.13-back-code-auditor.md`; achado não bloqueante: executor escreveu em `docs/STATUS.md` (pendência de operador — "configurar template de e-mail Reset Password") — território que pertence ao memory-manager, conteúdo correto, registrado como desvio de processo para `docs/Lessons-Learned.md` na próxima retrospectiva de stage). `security-auditor` APROVADO escalado para Opus a pedido do operador (superfície de autenticação), veredito em `.maestro/tmp/verdicts/4.12-4.13-back-security-auditor.md`, análise extensa sem achado bloqueante mas com 4 observações registradas para decisão futura: (O1) `redirectTo` monta origem a partir do header `host` sem allowlist — inofensivo hoje porque action exige sessão da própria vítima, mas fica frágil se fluxo futuro de "esqueci minha senha" sem sessão reaproveitar o helper (impacto: redirecimento abusivo; mitigação: allowlist de origem na config se futuro fluxo anônimo vier); (O2) `definirNovaSenha` aceita qualquer sessão válida, não só a de recuperação — decisão do contrato, Supabase oferece "Secure password change" dashboard para exigir reautenticação recente, recomendado ligar se fluxo futuro de gestão de segurança escopado vir a existir (impacto: troca de senha sem reauth em sessão antiga; mitigação: feature flag Supabase ou re-prompt de password no cliente antes de chamar a action); (O3) `type` da query convertido para `EmailOtpType` sem validação — pré-existente da Task 13.3a, inofensivo (impacto: nenhum, Supabase rejeita tipo inválido); (O4) fallback de protocolo não cobre `127.0.0.1` — defeito de conveniência local, não segurança (impacto: erro ao devolver do link de reset local, sem TLS). `qa-engineer` APROVADO (1ª tentativa, 609/609 testes, 4 novos de `mensagemErroTrocaSenha`, veredito em `.maestro/tmp/verdicts/4.12-4.13-back-qa-engineer.md`). Sem `ux-auditor` (backend puro, sem UI). Sem migration. **Desbloqueia a Task 4.12–4.13 (front).**
- **Task 4.12–4.13 (front)** ✅ (2026-08-13) — Segurança e troca de senha (RF-31): nova seção `SecaoSeguranca` em `components/perfil/PerfilLab.tsx` replicando o padrão visual de `SecaoOrganizacao`/`SecaoPerfilPessoal`, campo de e-mail read-only (mesmo aviso de "não editável — identificador da conta" já existente em `SecaoPerfilPessoal`). E-mail **movido** de `SecaoPerfilPessoal` para `SecaoSeguranca` (refactoring: `SecaoPerfilPessoal` fica só com nome/telefone/foto de perfil, e-mail é identidade de conta, não dado pessoal). Botão "Trocar senha" chama `solicitarTrocaSenha()` (Task 4.12-4.13 back) com `Dialog` de confirmação inline. Formulário "Definir nova senha" (dois campos: nova senha + confirmação, validação client-side via `senhaValida`/`senhasConferem`/`TAMANHO_MINIMO_SENHA` reaproveitadas de `lib/auth/validacao.ts`) renderizado só quando URL tem `?definirSenha=1` (usuário voltou do link de confirmação por e-mail da Task back), propagado via `searchParams` do Server Component `app/(app)/perfil/page.tsx` até `SecaoSeguranca` sem segunda leitura de sessão no client. Ao confirmar nova senha com sucesso, `router.replace("/perfil")` remove o parâmetro da URL (limpa o estado de formulário). `SecaoOrganizacao` não foi tocada. `code-auditor` APROVADO (1ª tentativa, veredito em `.maestro/tmp/verdicts/4.12-4.13-front-code-auditor.md`). Sem `security-auditor` (consumo de Server Actions já auditadas na Task back, nenhuma lógica de autenticação nova no client). `qa-engineer` APROVADO (1ª tentativa, 609/609 testes, sem regressão — task é wiring de UI sem função pura nova, veredito em `.maestro/tmp/verdicts/4.12-4.13-front-qa-engineer.md`). `ux-auditor` APROVADO (1ª tentativa, 3 screenshots via `/dev/preview/perfil` e `?definirSenha=1`, Design-System conformes, confirmando os 3 estados: seção segurança vazia, modal de confirmação, formulário "Definir nova senha", veredito em `.maestro/tmp/verdicts/4.12-4.13-front-ux-auditor.md`). Impacto Visual: Leve. Sem migration. **Fecha RF-31 (troca de senha com confirmação por e-mail) ponta a ponta** — junto com a Task 4.12-4.13 (back).

**Lote 4 — 13/13 tasks concluídas** ✅ (2026-08-14)
...
|---|---|---|---|---|---|---|---|
| 5.1–5.4 | Biblioteca e Editor de Módulo migram para o shell/Design System v3 (sidebar+topbar presentes nas duas telas), remoção dos textos clicáveis herdados da versão antiga (editor, calculadora, biblioteca, catálogo) | ✅ Completo | nenhuma | frontend-engineer (web) | Sonnet | **Completo** (retrofit de duas telas inteiras) | Design-System §6, §15.4; PRD RF-35 |
| 5.5–5.6 | Remover o card "validação tier 1 + tier 2" e varrer a UI por outros termos de especificação vazados (fora do rename "Preset"→"Módulo", já coberto pela Task 2.13) | ✅ Completo | nenhuma | frontend-engineer (web) | Haiku | **Leve** (ajuste pontual de texto) | Design-System §15.3; PRD RF-36 |
| 5.7–5.9 | Dashboard/orçamentos recentes: remover prazo de entrega, adicionar valor final do projeto e custo | ✅ Completo | nenhuma | frontend-engineer (web) | Sonnet | **Leve** (ajuste de card existente) | Modelo 11.5, 5.5; PRD RF-33 |
...
| 5.10 (front) | Seletor manual das etapas não-terminais (`novo`, `visita_agendada`, `projeto_3d`, `aguardando_aprovacao`) na tela do orçamento, com movimento livre entre elas (T1) — na prática, `visita_agendada`/`projeto_3d` dependem inteiramente deste seletor (não têm gatilho automático), enquanto `novo`/`aguardando_aprovacao` já são atingidas automaticamente por outras ações (criação do orçamento e "Gerar proposta", respectivamente) mas continuam selecionáveis à mão aqui, para corrigir um avanço/recuo feito por engano (T1 permite ida e volta livre); sem seletor (read-only) quando `etapaEsteira === "fechado"` — a única saída é o botão Reabrir (Task 1.9), restrito a `admin`. Badge do card de orçamento (dashboard e cabeçalho do orçamento) passa a usar `rotuloDoCard(status, etapaEsteira)` (Q-15): "Fechado" no terminal, "Em andamento" nas três intermediárias, rótulo do `status` comercial quando `novo` — substitui os badges "Em andamento"/"Fechado" que hoje não tinham origem | ✅ Completo | 5.10 (back) | frontend-engineer (web) | Sonnet | **Leve** (troca de origem de um badge já existente + seletor pontual, não é tela nova) | Modelo 7.2 (Q-15, T1, `rotuloDoCard`, exemplo trabalhado); PRD RF-33, Q-15 (resolvida) |
...
- **Task 5.5–5.6** ✅ (2026-08-14) — Remover vocabulário de especificação vazado na UI (RF-36): título do card de validação em `components/ambientes/AmbientesLab.tsx` mudou de "Validação (Tier 1 + Tier 2)" para "Validação"; rótulo "Bancada (mm)" virou "Meio (mm)" em 2 formulários (card "Alturas do perfil" e override de parede via `CAMPOS_ALTURA`); rótulo "Pé-direito (mm)" virou "Limite superior do aéreo (mm)" nos mesmos 2 lugares. Identificadores de domínio (`alturaBancada`, `peDireito`) e `id`/`htmlFor` dos inputs preservados — só texto visível mudou. Varredura confirmou nenhum outro vazamento de "Tier 1/2", "Template" ou "Preset". `code-auditor` APROVADO (1ª tentativa real), `qa-engineer` APROVADO (1ª, 5 capturas), `ux-auditor` APROVADO (1ª, 5 capturas em `.maestro/tmp/screenshots/`). Sem `security-auditor` (troca de texto, sem RLS/segredo). Impacto Visual: Leve.
- **Task 5.7–5.9** ✅ (2026-08-14) — Dashboard troca prazo de entrega por valor final e custo (RF-33): coluna "Prazo de entrega" removida da tabela "Orçamentos recentes" (`components/dashboard/DashboardView.tsx`); colunas "Valor final" (`resumo.precoFinal`) e "Custo" (`resumo.custoMaterial`, não custo total) adicionadas, formatadas com `formatarMoeda` + `tabular-nums`. `lib/dashboard/orcamentos.ts` ganhou `calcularResumoDoOrcamento` server-side, mirror do pipeline já usado pela aba Financeiro (`carregarEstadoAmbiente` → `calcularEngineOrcamento` → grupo único → `ratearPrecificacao`), com `try/catch` isolado por orçamento (falha de um item não derruba a lista) e catálogo buscado uma única vez por carregamento (não N vezes). Novo arquivo `lib/produto/buscarServer.ts` (variante server-side de `buscarCatalogoReal`, mesma lógica de mapeamento, troca só o client Supabase). Tipo `OrcamentoDashboardRow` perde `prazoEntrega`, ganha `valorFinal`/`custoMaterial` (`number | null`). Orçamento sem ambientes/itens mostra "—". 4 testes novos em `lib/dashboard/orcamentos.test.ts`, 637/637 passando. `code-auditor` APROVADO (1ª), `qa-engineer` APROVADO (1ª), `ux-auditor` APROVADO (1ª). Sem `security-auditor` (leitura server-side já escopada por RLS existente). Impacto Visual: Leve. **Nota deliberada (ponytail):** valor exibido sempre recalculado ao vivo, mesmo para orçamento congelado — não soma `linhaProposta.valorRateado` persistido. Diverge em tese da invariante I4 ("congelado não retroage"), mas é aceito porque o Dashboard é visão geral, não documento legal — fonte de verdade congelada continua sendo aba Proposta/PDF. Upgrade path: se essa divergência incomodar (catálogo/kerf mudam após o congelamento), upgrade é somar `linhaProposta.valorRateado` quando `congeladoEm !== null` — não implementado agora, YAGNI.
- **Task 5.10 (front)** ✅ (2026-08-16) — Seletor de etapa de esteira + badge `rotuloDoCard` (RF-33, Modelo-de-Domínio Seção 7.2, Q-6/Q-15 resolvidas): função `rotuloDoCard(status, etapaEsteira)` em `components/dashboard/StatusOrcamentoBadge.tsx` (3 ramos: `fechado` → "Fechado"; `visita_agendada`/`projeto_3d`/`aguardando_aprovacao` → "Em andamento"; `novo` → rótulo do status comercial, reaproveitando `ROTULO`/`VARIANTE` já existentes). Badge em dois lugares: Dashboard (já existente, agora com etapa) e `/orcamento/[id]` (novo, ao lado de "Cliente: <nome>" em `components/orcamento/OrcamentoAbas.tsx`). Novo Client Component `components/orcamento/SeletorEtapaEsteira.tsx`: `<Select>` com 4 etapas não-terminais, oculta-se em read-only quando `etapa=fechado`, reverte seleção com erro inline em falha. `etapaEsteira` exposta em 3 pontos de leitura: `buscarOrcamentoPorId`, `buscarItemDoOrcamento` (ambos em `lib/orcamento/buscar.ts`) e `buscarDadosDashboard` (`lib/dashboard/orcamentos.ts`). 640/640 testes passando. **Correção de arquitetura (não foi achado de gate)**: Server Action `atualizarEtapaEsteira` usava `"use server"` inline em arquivo que também exportava funções síncronas (`lib/orcamento/etapa-esteira.ts`), quebrando import por Client Component (regra Next.js). Extraída para arquivo novo `lib/orcamento/atualizarEtapaEsteira.ts` com `"use server"` no topo; `lib/orcamento/etapa-esteira.ts` mantém só síncronas. `code-auditor` APROVADO (1ª), `qa-engineer` APROVADO (1ª), `ux-auditor` APROVADO (1ª, 8 screenshots). Sem `security-auditor` (Server Action já auditada na Task 5.10-back). Sem migration. Impacto Visual: Leve.
- **Task 5.1–5.4** ✅ (2026-08-17) — Migração de `/biblioteca` e `/modulo` para shell/Design System v3 (RF-35): **investigação prévia confirmou que `/biblioteca` já estava 100% migrada desde Task 13.7c** (sem rótulo legado, sem CSS antigo), sem mudança necessária lá. Escopo real: `/modulo` — rota movida de `app/modulo/page.tsx` para `app/(app)/modulo/page.tsx` (mesma URL pública, agora dentro do shell com sidebar/topbar), wrapper legacy `<div className="wrap"><header className="top">` removido (shell já provê container via `main p-xl`), 3 links de texto herdados ("← calculadora", "Biblioteca de módulos", "Catálogo") removidos, navegação fica apenas pela sidebar. Entrada em `TITULOS_POR_ROTA` (`components/shell/Topbar.tsx`) para exibir "Editor de módulo". Harness novo sem sessão `app/dev/preview/modulo/page.tsx` (404 em produção), registrado em `lib/auth/rotas.ts`. Comentários stale em `middleware.ts`/`lib/auth/rotas.ts` atualizados. **Gap deliberado registrado**: `EditorItemNucleo.tsx` e componentes-filho (`CaixaCard.tsx`, `DivisoesCard.tsx`, `PortasCard.tsx`) continuam com CSS legado (`.card`/`.legado-grid`, `app/globals.css`) por compartilharem núcleo com `/orcamento/[id]/item/[itemId]` em produção — retrofit desse núcleo é risco maior, candidato a task futura de UI. `code-auditor` APROVADO (1ª), `qa-engineer` APROVADO (1ª, 640 testes), `ux-auditor` APROVADO (1ª, 6 screenshots). Sem `security-auditor` (rota/layout, sem RLS/segredo). Impacto Visual: Completo.

**Lote 5 — 5/5 tasks concluídas** ✅ (2026-08-17)
